...
| `LEASH_FACILITATOR_URL`            | _seller's choice_               | Override the facilitator the **seller** uses to verify and settle. Set to `https://facilitator-devnet.leash.market` to route through the Leash-operated devnet facilitator, or `http://localhost:8787` for a self-hosted one (see [Run a Facilitator](../../apps/docs/guides/run-a-facilitator.mdx)). The buyer never speaks to the facilitator directly — this is documented here because changing it on the seller flips it for the whole settlement. |
