...
export async function createAutomation(
  ...
  await execute(
    ...
    `INSERT INTO automations (
      id, owner_privy_id, agent_mint, name, description, instructions, status,
...
export async function getAutomationById(db: DbClient, id: string): Promise<AutomationRow | null> {
  const res = await execute(
    db,
    `SELECT * FROM automations
...
export async function getAutomationRunById(
  ...
  const res = await execute(db, `SELECT * FROM automation_runs WHERE id = ? LIMIT 1`, [id]);
  const row = res.rows[0];
...
export async function createAutomationRun(
  ...
  await execute(
    ...
    `INSERT INTO automation_runs (
      ...
      trigger_payload, status, source_summary, spend_usd, receipts,
      idempotency_key, claimed_by, started_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      input.automationId,
...
export async function finishAutomationRun(
  ...
  await execute(
    ...
    [
      ...
      JSON.stringify(input.deliveryResult ?? {}),
      input.spendUsd ?? '0',
      JSON.stringify(input.receipts ?? []),
      finishedAt,
      input.runId,
  ...
  await execute(
    ...
    `UPDATE automations
     SET last_run_at = ?,
         ...
         updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')
     WHERE id = ?`,
    [finishedAt, input.status, input.status, input.nextRunAt ?? null, input.automationId],
  );
  return getAutomationRunById(db, input.runId);
}
...
export async function claimNextDueAutomation(
  ...
  const lockedUntil = new Date(new Date(now).getTime() + lockMs).toISOString();
  ...
  const id = due.rows[0] ? String((due.rows[0] as Record<string, unknown>).id) : null;
  if (!id) return null;
  ...
  const claimed = await execute(
    ...
    `UPDATE automations
     ...
     WHERE id = ?
       ...
       AND (next_run_at IS NULL OR next_run_at <= ?)
       AND (locked_until IS NULL OR locked_until <= ?)`,
    [input.workerId, lockedUntil, id, now, now],
  );
