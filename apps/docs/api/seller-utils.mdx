...
description: HTTP parity for the @leash/seller-kit helpers — networks, facilitator, parse-price, pay-to.
---
...
```json
{
  "items": [
    ...
    { "network": "solana-mainnet", "caip2": "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpKuc", "...": "..." }
  ],
...
the public default for the cluster (`facilitator.svmacc.tech` on
devnet, `facilitator.payai.network` on mainnet).
...
'@leash/seller-kit'`. The HTTP surface exists for two reasons:
...
2. **UIs.** Dashboards, internal tools, and the explorer itself
   ...
   `@leash/core` registry into the browser.

## See also

- [`@leash/seller-kit`](/sdk/seller-kit) — the TypeScript surface
