...
Brand-new agent creation (`prepareAgentMint`) is not yet exposed as an
HTTP endpoint — mint the asset via the SDK, then start hitting
`/v1/agents/{mint}/...` from the second action onwards.
...
```bash
# 0. Mint the agent asset locally with @leash/registry-utils
#    (no API endpoint yet — see /sdk/registry-utils#prepareAgentMint).
#    Save the resulting `mint` pubkey as $MINT.
