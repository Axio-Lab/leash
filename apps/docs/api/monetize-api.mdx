...
- **TypeScript:** [`createAgent`](/sdk/registry-utils) or
  ...
  `@leash/registry-utils` — same on-chain result as the API.
...
[`@leash/seller-kit`](/sdk/seller-kit) exposes `createSeller(app,
options)` — real `@x402/hono` middleware, Asset Signer PDA as `payTo`,
optional receipt POST to the Leash API when both env vars are set.
...
**How forwarding works:** when `LEASH_API_URL` and `LEASH_API_KEY` are
set, seller-kit's `onReceipt` hook POSTs each settled `ReceiptV1` to
...
vars and receipts stay local (`receipts: false` is explicit opt-out).
...
[buyer endpoints](/api/buyer) mirror `@leash/buyer-kit`:
...
- **Receipt feed** — `GET /v1/receipts/{agent}`; verify with
  `@leash/core` or [`POST /v1/buyer/receipt/verify`](/api/buyer).
