...
createSeller(app, {
  ...
  routes: [
    { method: 'GET', path: '/quote', price: { amount: '0.01', currency: 'USDC' } },
  ],
...
- **Refunds.** Coming in Phase 6 — the API will expose
  `POST /v1/payments/refund` that issues an inverse transfer and
  writes a `refund` receipt linked to the original call.
