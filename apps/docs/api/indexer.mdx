...
- **Any `prepare*` call against `/v1/agents/{mint}/...`** — adds the
  asset and treasury PDA.
- **`provision/prepare`, `withdraw/prepare`, `withdraw-all/prepare`** —
  also add every relevant `treasury_ata`.
- **`GET /v1/agents/{mint}/treasury/balances`** — adds the PDA + every
  SPL ATA the snapshot surfaces. This is the cheapest "force-register
  this agent" call available; it's read-only, free, and side-effects
  all three watch kinds at once.
- **`POST /v1/receipts/{agent}` and `POST /v1/agents/{mint}/pull-target`**
  — register the asset (no ATA discovery, since neither call needs it).

The indexer additionally self-bootstraps on startup: for every
...
shows up?", see [Explorer tracking](/api/explorer-tracking).
...
our end — open an incident in [Discord](https://discord.gg/leashmarket). The
