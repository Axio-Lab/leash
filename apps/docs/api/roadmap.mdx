...
indexer → events → webhooks → metrics loop, plus the first identity-profile
surface for handles, verified domains, capability cards, claims, and public
verification.
...
| **Selective disclosure**            | v1 shipped | Public/private visibility controls for capability cards and claims.                                                             |
| **Delegated operator history**      | next       | Show executive rotations, revoked delegates, pause state, and operator changes over time from indexed events.                   |
| **Agent-to-agent verification API** | next       | Extend `/v1/identity/verify` into a richer policy/reputation decision for automated buyers.                                     |
