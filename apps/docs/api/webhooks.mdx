...
A first-party `verifySignature` helper ships in `@leash/api-client`
(Phase 6 codegen) and a reference implementation lives at
`apps/api/src/webhooks/sign.ts` in the monorepo.
