...
A reference implementation lives at `apps/api/src/webhooks/sign.ts`
in the monorepo. First-party `verifySignature` helpers in the
generated polyglot SDKs are on the roadmap and not yet shipped — for
now, copy the snippet above (or the monorepo file) into your service.
...
In dev (`apps/api/src/dev.ts`) the worker runs in-process every two
seconds. For staging / prod, run it as a dedicated worker so a slow
receiver can't backpressure the API server. The same module is
exported as `startWebhookWorker(db, intervalMs, options)` from
`@leash/api/webhooks/worker` — wire it into your own process supervisor.
