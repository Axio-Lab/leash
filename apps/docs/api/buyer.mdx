...
description: HTTP parity for @leashmarket/buyer-kit — quote, policy gate, payment prepare/execute, receipt finalize/verify.
---
...
TypeScript [`@leashmarket/buyer-kit`](/sdk/buyer-kit) one-to-one:
...
A pure HTTP wrapper around `evaluate` from `@leashmarket/core`. No IO, no
...
<Note>
  **Indexer follow-up after `/payment/execute`.** This call
  auto-registers the **buyer** agent (`body.agent`) on the indexer
  watchlist, so the buyer's spend receipts and any future on-chain
  activity for the buyer surface on the explorer automatically.

It does **not** register the **seller's** treasury. If the seller
...
idempotent, free, and registers the PDA + every ATA in one shot.
...
See [Explorer tracking → Sequence 2](/api/explorer-tracking#sequence-2--issue-a-buyer-payment-against-an-x402-endpoint)
...
whole `@leashmarket/core` library in your runtime.
...
Same byte output as `finalizeReceipt` from `@leashmarket/core`. Idempotent.
...
- [`@leashmarket/buyer-kit`](/sdk/buyer-kit) — the TypeScript surface
