...
- **Race-free reads.** The `event_id` exists *before* the transaction
...
```json
{
  ...
  "echo": { /* function-specific extras (asset, collection, ATA, etc.) */ }
}
```
...
- **Broadcasts** via the API's RPC (Helius by default).
