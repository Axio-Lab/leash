...
There is no API endpoint for **minting a brand-new agent** yet — that
flow uses `prepareAgentMint` directly from
[`@leash/registry-utils`](/sdk/registry-utils). Once the asset exists on
chain, every other lifecycle action above is callable over HTTP.

<Note>
  **Indexer follow-up after your first agent action.** Every
  `/v1/agents/{mint}/...` prepare endpoint auto-registers the agent
  asset and treasury PDA on the indexer watchlist (via
  `ensureWatched`). That's enough for the explorer to surface
  withdraws, identity changes, delegations, and token updates.
...
```json
{
  ...
  "network": "solana-devnet",
  "transaction": null,
  "echo": { /* same shape as the active envelope */ },
  "no_op": true
}
```
