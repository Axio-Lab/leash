---
title: 'Payment links'
description: Hosted x402 or MPP payment links at /x/{id} for identity-linked paid capabilities.
---
...
either a response template or a stored upstream URL to forward to after payment.
Optional **`protocol`**
...
For **`protocol: "x402"`** (default), buyers get a `402` with x402
`PAYMENT-REQUIRED`. For **`"mpp"`**, the unpaid response uses **`application/problem+json`**
(MPP challenge); [`@leashmarket/buyer-kit`](/sdk/buyer-kit) detects and settles either shape.
See [MPP on Solana (Leash)](/standards/mpp-on-solana).

The link runs as a real gated HTTP resource. Buyers (or any agent driving
[`@leashmarket/buyer-kit`](/sdk/buyer-kit)) probe, pay, replay, and receive your
...
To turn an API you already host into a payable endpoint, store it in
`metadata.upstream_url`. The `response` remains a fallback/template for reads and
...
At runtime the first call to `/x/{id}` returns payment instructions. The paid
...
| `metadata`           | No       | Free-form JSON returned on every read. Set `upstream_url` to forward paid calls.    |
...
The API hands the signed tx to the configured facilitator and the
facilitator settles the SPL transfer on chain. If the link has
