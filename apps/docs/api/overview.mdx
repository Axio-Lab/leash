...
- **Polyglot.** The SDK is TypeScript-only. The API is HTTP + OpenAPI,
  ...
  `curl`.
- **No Solana ops.** You never need to run an RPC node, manage Helius
  rate limits, or patch a `umi-bundle-defaults`. The API uses our pool
...
| ------------------------ | ----------------------------------------------------------------------- |
| `api.leash.market`       | This API. Hono + zod-openapi behind Cloudflare. JSON 3.1 spec at `/openapi.json`. |
| `docs.leash.market`      | This site (Mintlify). API reference is generated from the same spec.    |
| `explorer.leash.market`  | Standalone protocol explorer; reads from `api.leash.market` only.       |
...
| Prefix       | Network          | Default RPC                      |
| ------------ | ---------------- | -------------------------------- |
| `lsh_test_*` | `solana-devnet`  | `LEASH_API_RPC_DEVNET`           |
| `lsh_live_*` | `solana-mainnet` | `LEASH_API_RPC_MAINNET`          |
