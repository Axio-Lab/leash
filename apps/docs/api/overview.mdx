...
- **Receipt + event surface for free.** Every prepare, every submit,
  ...
  your account.
- **Explorer integration.** Anything the API sees — specifically
  every transaction submitted via `POST /v1/submit` — shows up in
  the explorer with no extra work. Transactions broadcast outside
  the API (raw RPC) are not tracked.
...
| Agents        | `POST /v1/agents/prepare` (mint a new agent), `GET /v1/agents/{mint}`, `GET /v1/agents/{mint}/treasury/balances`                                                                      |
| Prepare       | 14 routes mapped 1:1 to the SDK's `prepare*` helpers — see [Prepare → Submit](/api/prepare-submit)                                                                                    |
...
| Admin         | `POST/GET /v1/admin/api-keys` (create **requires** `owner_wallet`; list filter `owner_wallet=`; older rows return `null`), `POST /v1/admin/api-keys/{id}/disable` — admin-only        |
...
- [Payment links](/api/payment-links) — hosted x402 paywalls served
  at `/x/{id}` for identity-linked paid capabilities.
- [Seller utilities](/api/seller-utils) — `parse-price`,
  `facilitator`, `networks`, `pay-to`. Mirrors the
  `@leashmarket/seller-kit` exports for polyglot SDKs.
- [Buyer endpoints](/api/buyer) — full HTTP parity with
  `@leashmarket/buyer-kit`: quote, policy gate, prepare, execute,
...
- [Explorer tracking](/api/explorer-tracking) — the per-event-kind
  cookbook: exactly which endpoint to call so each row shows up on
  the explorer and in `/v1/metrics/events`.
