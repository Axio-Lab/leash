...
| `leash_pay_payment_link` | Pay an x402 payment link with the agent's treasury |
...
Sends a probe request and returns whether the URL is a valid x402 paywall,
along with the parsed `payment-required` header details.

```ts
const result = await probePaymentLink('https://api.example.com/premium');
if (result.ok) {
  console.log(result.requirement); // { maxAmountRequired, asset, payTo, ... }
}
```
