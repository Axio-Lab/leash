...
Before paying a seller you can verify its Leash identity selector with the
lightweight helper exported from the same package:

```ts
import { verifyAgentIdentity } from '@leashmarket/buyer-kit';

const verdict = await verifyAgentIdentity({
  selector: { domain: 'seller.example' },
});

if (!verdict.verified) throw new Error('seller identity did not verify');
```
