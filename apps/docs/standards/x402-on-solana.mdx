...
<Info>
  Leash also supports **MPP** (problem+json challenges + facilitator `mpp/settle`) on the same agents and treasuries. See [MPP on Solana (Leash)](/standards/mpp-on-solana).
</Info>
