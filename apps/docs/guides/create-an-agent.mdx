...
| `registerAgentIdentity` (BYO) | Attaches an Agent Identity to an MPL Core asset you already own; you host the registration JSON yourself | You already minted the asset, or you need full custody of the registration document (Pinata, Arweave, your own gateway) |
...
const { assetAddress, signature, network } = await createAgent(umi, {
  ...
  uri: 'https://gateway.pinata.cloud/ipfs/<cid>', // on-chain Core metadata
...
- `/agents/new` — Create Agent form, presented as a **two-step tabbed wizard**
  ...
  - **Step 1 — Identity.** Two metadata modes:
    - **Pin to Pinata for me** (default): the form builds `{ name, description, image? }`, POSTs it to `/api/registry/pin` (server-side `PINATA_JWT`), and uses the returned IPFS gateway URL as the on-chain Core URI. Set `PINATA_JWT` in `apps/web/.env.local` to enable.
    - **I have a URI**: paste an existing publicly-fetchable JSON URL (Arweave / IPFS / HTTPS) and skip the upload step.
