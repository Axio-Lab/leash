...
See [Run an Agent](https://www.metaplex.com/docs/agents/run-an-agent) for the full execution model. The actual `Execute` composer (e.g. SPL `transferChecked` from the treasury) is the next item on the roadmap; for now the playground stops at delegation.
