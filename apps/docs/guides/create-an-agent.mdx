...
description: Mint a Core asset and register an Agent Identity in one transaction with @leashmarket/registry-utils.
---

Leash agents follow Metaplex's [MIP-104 / Agent Identity](https://www.metaplex.com/docs/agents/mint-agent) standard: a single MPL Core asset with an `AgentIdentity` plugin attached. `@leashmarket/registry-utils` ships a thin wrapper around `mintAndSubmitAgent` that does both in one transaction via the hosted Metaplex API, so you don't have to sequence two instructions yourself.
...
```ts
import { prepareAgentMint, sendPreparedAgentMint } from '@leashmarket/registry-utils';
...
```ts
import { registerExecutive, delegateExecution } from '@leashmarket/registry-utils';
...
```ts
import { setSpendDelegation } from '@leashmarket/registry-utils';
...
The Next.js playground (`apps/playground`) wraps all of this in a UI **signed by your connected Privy wallet** — there is no shared hot key:
...
- `/agents/[mint]` → **Token (Genesis)** tab — paste the token image URL Metaplex accepts, then launch (see [Launch an agent token](/guides/launch-agent-token)).

Configure `NEXT_PUBLIC_PRIVY_APP_ID` in `apps/playground/.env.local`, log in via the wallet button (top-right), and fund your embedded wallet on devnet (`solana airdrop 1 <pubkey> --url devnet`). The bridge is `apps/playground/lib/privy-umi.ts` — `usePrivyUmi()` wraps the Privy `ConnectedSolanaWallet` in Metaplex's `walletAdapterIdentity` so any `@leashmarket/registry-utils` SDK call works straight from a React component.
