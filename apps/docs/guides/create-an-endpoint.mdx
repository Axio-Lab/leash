...
### Defaults by network (`@leash/core`)

| Network (`solana-*`) | Default facilitator URL             | Notes                                              |
| -------------------- | ----------------------------------- | -------------------------------------------------- |
| `solana-devnet`      | `https://facilitator.svmacc.tech`   | Gas-sponsored devnet; Token-2022 + `exact` scheme. |
| `solana-mainnet`     | `https://facilitator.payai.network` | Gas-sponsored mainnet `exact` scheme (PayAI).      |

Override for **any** buyer/seller process: set `LEASH_FACILITATOR_URL` or pass `facilitator: 'https://…'` into `createBuyer` / `createSeller`. Resolution order is documented in [`defaultFacilitatorFor`](https://github.com/leash-market/leash/blob/main/packages/core/src/x402/facilitator.ts).
...
### Self-host Leash’s facilitator (devnet today; mainnet capable)

To run your own binary, wire devnet vs mainnet with `LEASH_FACILITATOR_NETWORKS`, fund a **dedicated** fee-payer keypair (never reuse the buyer’s signing key — x402 rejects `fee_payer_transferring_funds`), and front the listener with TLS on mainnet. Full walkthrough: [Run a Leash facilitator](/guides/run-a-facilitator).
