...
Third-party examples: `facilitator.svmacc.tech` (devnet), `facilitator.payai.network` (mainnet), community deployments, or your own self-hosted instance. Resolution order: [`defaultFacilitatorFor`](https://github.com/leash-market/leash/blob/main/packages/core/src/x402/facilitator.ts).
...
The Leash defaults work for most users. If you need a custom settlement path — self-hosted [`@leash/facilitator`](/guides/run-a-facilitator), a community host, or a third-party provider like `facilitator.svmacc.tech` / `facilitator.payai.network` — pass a `facilitator:` override or set the env var. The wire contract is the same across all of them. See [Run a Leash facilitator](/guides/run-a-facilitator).
