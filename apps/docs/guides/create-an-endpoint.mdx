...
### Built-in defaults by network (`@leash/core`)

If you do not pass a `facilitator` URL, Leash picks a **sensible public default** per cluster so a new project works out of the box. Those defaults are **not exclusive** — they are just two well-known hosts that implement the same x402 + Solana **exact** SVM surface as everyone else.

| Network (`solana-*`) | Default URL (when unset)            | Who runs it (examples)         |
| -------------------- | ----------------------------------- | ------------------------------ |
| `solana-devnet`      | `https://facilitator.svmacc.tech`   | SVMacc (gas-sponsored devnet). |
| `solana-mainnet`     | `https://facilitator.payai.network` | PayAI (gas-sponsored mainnet). |

### Other facilitators (you are not locked in)

**Any** HTTPS base URL that implements the x402 facilitator contract your kits already speak — `GET /supported`, `POST /verify`, `POST /settle` — and advertises the **exact** SVM scheme for the network you are on can be used instead. Point at it with:

- `facilitator: 'https://your-facilitator.example.com'` on `createBuyer` / `createSeller`, or
- `LEASH_FACILITATOR_URL` / `NEXT_PUBLIC_LEASH_FACILITATOR_URL` / `LEASH_API_FACILITATOR_URL` (depending on surface), or
- the hosted **Leash** binary at `https://facilitator.leash.dev` (devnet v0.1) or **self-hosted** [`@leash/facilitator`](/guides/run-a-facilitator) on `http://localhost:8787` or your own domain — same wire shape as third-party facilitators from the x402 ecosystem (e.g. other PayAI / Coinbase / community deployments), as long as they expose a compatible `/supported` for your cluster.

Resolution order for SDK defaults is in [`defaultFacilitatorFor`](https://github.com/leash-market/leash/blob/main/packages/core/src/x402/facilitator.ts).
...
Both are optional; if unset, the app uses the same defaults as the table above. Point both at `http://localhost:8787` when running [`@leash/facilitator-app`](/guides/run-a-facilitator) locally.
...
### Self-hosting the Leash facilitator binary (optional)

`@leash/facilitator` is **one** supported backend — useful when you want an operator-controlled devnet host (`https://facilitator.leash.dev`) or a private instance (`LEASH_FACILITATOR_NETWORKS`, dedicated fee payer, TLS on mainnet). It is wire-compatible with any other x402 SVM facilitator; pick whichever host you trust. Walkthrough: [Run a Leash facilitator](/guides/run-a-facilitator).
