...
3. Fill in:
   ...
   - **Method** — `GET` or `POST`.
   - **Price** — any `@x402/svm`-parseable string (`$0.001`, `0.01 USDC`, `0.5`).
   ...
   - **Post-payment hooks** (all optional, see below):
     - `redirect_url` — 303 redirect after payment (e.g. thank-you page).
...
Once a buyer pays, the seller has three orthogonal ways to react:

| Hook            | When you'd use it                                                         |
| --------------- | ------------------------------------------------------------------------- |
| `redirect_url`  | Funnel the buyer to a thank-you page, gated download, or product page.    |
...
### `redirect_url` — 303 to a thank-you page

Set `redirect_url: "https://yoursite.com/thank-you"` on the endpoint and
the seller will respond with `303 See Other` after settlement, with the
payment proof appended:

```text
HTTP/1.1 303 See Other
Location: https://yoursite.com/thank-you?leash_tx=<sig>&leash_receipt=<hash>&leash_agent=<mint>
```

The destination page can verify the payment server-side (look up the
receipt under `/a/<agent>/receipts/<hash>`) before unlocking content.

### `webhook_url` — fire-and-forget agent-to-agent

Set `webhook_url: "https://your-agent.com/leash-callback"` and after
each settlement the runner will POST:
...
request header to fire one of their own webhooks. This is on top of the
seller's `webhook_url` (both will be called) — so the seller and buyer
can each pipe the response into different downstream agents.
...
read response headers separately. Ignored when `redirect_url` is set or
...
```jsonc
{
  ...
  "redirect_url": "https://yoursite.com/thank-you",   // optional
