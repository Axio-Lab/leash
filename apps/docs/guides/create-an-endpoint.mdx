...
description: No-code x402 paid capabilities. Pick an agent identity, declare a price + response, share a URL.
---

The Leash playground ships a **Payment-Link Builder** for identity-linked paid
capabilities. You don't need to host a Hono server, manage facilitator wiring,
or write any code; the runner stores your `EndpointV1` descriptors and the
Next.js app exposes them at
...
3. Fill in:
   ...
   - **Upstream URL** (optional) — an existing API URL to call after settlement.
     If present, paid calls forward to this URL and return its live response.
   - **Expected request body** (optional, POST only) — arbitrary JSON metadata
     that describes the body buyers should send to the hosted payable URL.
     This is not the live request body; the buyer sends the real body when
     paying/calling `/x/{id}`.
...
| -------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------- |
| `solana-devnet`      | `https://facilitator-devnet.leash.market` | Leash-operated devnet facilitator. Gas-sponsored; Token-2022 + `exact` SVM scheme. |
...
Third-party examples: `facilitator-devnet.leash.market` (devnet), `facilitator.leash.market` (mainnet), community deployments, or your own self-hosted instance. Resolution order: [`defaultFacilitatorFor`](https://github.com/leash-market/leash/blob/main/packages/core/src/x402/facilitator.ts).
...
The Leash defaults work for most users. If you need a custom settlement path — self-hosted [`@leashmarket/facilitator`](/guides/run-a-facilitator), a community host, or a third-party provider like `facilitator-devnet.leash.market` / `facilitator.leash.market` — pass a `facilitator:` override or set the env var. The wire contract is the same across all of them. See [Run a Leash facilitator](/guides/run-a-facilitator).
...
- Settles via [`facilitator-devnet.leash.market`](https://facilitator-devnet.leash.market)
