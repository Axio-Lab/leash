...
The Leash defaults work for most users. If you need a custom settlement path — self-hosted [`@leash/facilitator`](/guides/run-a-facilitator), a community host, or a third-party provider like `devnet-facilitator.leash.market` / `facilitator.leash.market` — pass a `facilitator:` override or set the env var. The wire contract is the same across all of them. See [Run a Leash facilitator](/guides/run-a-facilitator).
