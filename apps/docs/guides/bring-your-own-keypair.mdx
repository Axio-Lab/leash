...
The web playground (`apps/playground`) signs everything with a connected Privy embedded wallet because that's the lowest-friction path for trying Leash from a browser. The SDK itself has no Privy dependency — `@leashmarket/registry-utils` only cares that `umi.identity` can sign Solana transactions. This guide shows the three production-grade alternatives.
...
The first three all reduce to the same SDK pattern: build a Umi, install a `keypairIdentity` plugin, then call `@leashmarket/registry-utils` exactly the same way. Only the source of bytes differs.
...
import { generateOperatorKeypair, exportOperatorJson } from '@leashmarket/registry-utils';
...
import { createAgent, importOperatorJson } from '@leashmarket/registry-utils';
...
```ts
// apps/playground/lib/umi.ts (excerpt)
...
```ts
import { generateOperatorKeypair, operatorRegistration } from '@leashmarket/registry-utils';
