...
//    `mint`). Mint + delegate locally with `@leash/mcp::mintAgentLocally`
...
> [`@leash/mcp::mintAgentLocally`](/agents/mcp) or the
...
<Note>
  **The SDK does not pay payment links** — paying requires Solana signing, which `@leash/sdk`
  deliberately avoids so it stays browser-friendly. To pay programmatically, use
  [`@leash/buyer-kit`](/sdk/buyer-kit) (heavyweight, signs locally) or the
  [`@leash/mcp`](/agents/mcp) host's `pay()` tool.
</Note>
