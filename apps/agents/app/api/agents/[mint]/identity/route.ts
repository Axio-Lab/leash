...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ mint: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'upstream_unreachable', detail: 'apps/api is offline; identity not loaded.' },
      { status: 503 },
    );
  }
}

export async function PUT(req: NextRequest, { params }: { params: Promise<{ mint: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'upstream_unreachable', detail: 'apps/api is offline; identity not updated.' },
      { status: 503 },
    );
  }
}
